...
# THE STALE BINARY IS REMOVED SO THE BUNDLER CANNOT REUSE IT.
#
# Measured 2026-08-09, and it cost most of a night. `cargo tauri build` bundled a
# `target/release/mind2t` compiled HOURS earlier, missing every source change since, and every
# assertion below passed against it - they check the executable NAME and the plist VERSION,
# neither of which moves when the code does. Orel ran an app whose panes opened in the wrong
# directory with the wrong shell while this script printed "installed" four times.
#
# The mechanism: tauri compiles with `--features tauri/custom-protocol`, so its artifact and a
# plain `cargo build --release` are DIFFERENT compilations sharing one output path, and cargo's
# fingerprint for tauri's variant went stale. Removing the path first makes reuse impossible -
# cargo must produce it again, or the freshness check below fails the build outright.
#
# A build product, regenerated by the very next command. Nothing here is unrecoverable.
rm -f "$ROOT/target/release/mind2t"
...
# THE FRESHNESS ASSERTION. The one check that would have caught the night this script cost.
#
# Compared BEFORE signing, because `codesign --force` rewrites the binary and changes its hash -
# which is also why hashing the INSTALLED copy against a build proves nothing and misled the
# diagnosis for an hour.
FRESH_SUM="$(shasum -a 256 "$ROOT/target/release/mind2t" | cut -d' ' -f1)"
BUNDLED_SUM="$(shasum -a 256 "$BUILD/Contents/MacOS/mind2t" | cut -d' ' -f1)"
[ "$BUNDLED_SUM" = "$FRESH_SUM" ] || {
  echo "the bundle does NOT contain the binary this build produced" >&2
  echo "  built:   $FRESH_SUM" >&2
  echo "  bundled: $BUNDLED_SUM" >&2
  exit 1; }
