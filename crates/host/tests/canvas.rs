...
use mind2t_vt_host::canvas::{Canvas, PaneSpec};
use mind2t_vt_host::layout::{Canvas as Grid, Rect};
...
fn a_multi_row_canvas_refuses_to_split() {
    ...
    assert!(
        matches!(refused, Err(mind2t_vt_host::canvas::CanvasError::NotSplittable { rows: 2 })),
...
fn splitting_stops_before_a_pane_becomes_unusable() {
        ...
        match canvas.split(&context, shell(&PaneSpec::shell()), FONT, None) {
            Ok(_) => {
                ...
                for (index, pane) in canvas.panes().iter().enumerate() {
                    let geometry = pane.session.geometry();
                    assert!(
                        geometry.cols >= mind2t_vt_host::canvas::MIN_SPLIT_COLS,
                        "split {attempt} left pane {index} at {} columns, below the {} floor",
                        geometry.cols,
                        mind2t_vt_host::canvas::MIN_SPLIT_COLS
                    );
                }
            }
            Err(error) => {
    ...
    assert!(
        matches!(refused, mind2t_vt_host::canvas::CanvasError::TooNarrow { .. }),
