...
fn expected() -> Vec<(String, Option<String>)> {
    let mut want = vec![
        ...
        (
            "TERM".to_string(),
            Some(mind2t_vt_host::launch::CHILD_TERM.to_string()),
        ),
    ];
    want.sort();
    want
...
fn dress_sets_the_terminal_type_and_removes_the_session_markers() {
    ...
    mind2t_vt_host::launch::dress(&mut command);
...
fn the_shell_command_path_is_dressed() {
    let command = mind2t_vt_host::launch::shell_command();
    assert_eq!(
        mutations(&command),
        expected(),
...
fn the_agent_path_is_dressed() {
    let agent = mind2t_vt_host::agent::REGISTRY
        .iter()
        .find(|agent| agent.id == "claude")
        .expect("the registry still lists claude");
    let command = mind2t_vt_host::agent::launch(agent, "claude", &[]).expect("a plain launch is allowed");
    assert_eq!(
        mutations(&command),
        expected(),
...
fn claude_typed_into_a_shell_pane_reports_a_clean_session() {
    ...
    let mut session = mind2t_vt_host::session::Session::spawn_fitted_on(
        &gpu,
        mind2t_vt_host::launch::shell_command(),
        1200,
        700,
        16.0,
        None,
...
fn every_spawn_path_starts_the_child_at_home() {
    ...
    mind2t_vt_host::launch::dress(&mut dressed);
    assert_eq!(dressed.get_current_dir(), Some(home.as_path()));

    assert_eq!(
        mind2t_vt_host::launch::shell_command().get_current_dir(),
        Some(home.as_path()),
    ...
    let agent = mind2t_vt_host::agent::REGISTRY
        ...
        .expect("the registry still lists claude");
    assert_eq!(
        mind2t_vt_host::agent::launch(agent, "claude", &[])
